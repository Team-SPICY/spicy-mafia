...
class LobbyViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        # Quiz questions
        quizQ = dict( 0: 'Which player should take a shot?', 1: 'Which player should finish their drink?', 2: 'Which player is most likely to accidentaly give away their role?', 3: 'Which player has the highest GPA?', 4: 'Which player is most likely to get blacked out playing Mafia?', 5: 'Which player is most likely to get called out by Gary for talking in class', 6: 'Which player is most likely to sleep through their CSE110 final exam?', 7: 'Which player has a scret crush on Gary G.?', 8: 'Which player has the prettiest eyes?', 9: 'Which player has the best smile?' 10: 'Which player gets to choose another player to take a shot?', 11: 'Which player is most likely to fail CSE 3 because they forgot they were taking it?', 12: 'Which player is most likely to forget what a binary search tree is during an interview?', 13: 'Which player is most likely to get a job offer from Google?', 14: 'Which player should take two shots?', 15: 'Which player is most likely to move out of their apartment and into the CSE basement labs?', 16: 'Which player has the biggest meatballs?', 17: 'Which player is most likely to put a horse head in your bed?', 18: 'Which player is most likely to actually use bubble sort?', 19: 'Which player is most likely to push their code before testing it?')
        lobby_keys = db.child('lobbies').shallow().get().val()
        ...
        while retry < 10:
            ...
            id = ''
            retry += 1
        data ={"lobbies/" + id: {"numMafia": 0, "numOther": 0, 'isActive': False, 'quizQuestions':quizQ }}
